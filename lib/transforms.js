...
function wrap (state, node, program) {
  ...
  if (operator === 'traceIterator' || operator === 'traceAsyncIterator') wrapper = wrapIterator(state, node, program)
...
/**
 * Builds the wrapper AST for an iterator-returning function.
 *
 * Wraps the iterator's `next`, `return`, and `throw` methods with a `:next`
 * tracing channel so each iteration step is traced independently.
 * Uses `traceSync` for sync iterators and `tracePromise` for async iterators.
 *
 * @param {{ channelName: string, operator: string }} state
 * @param {import('estree').Node} node
 * @param {import('estree').Program} program
 * @returns {import('estree').Program} Parsed wrapper function program.
 */
function wrapIterator (state, _node, program) {
  const { channelName, operator } = state
  const channelVariable = formatChannelVariable(channelName)
  const nextChannelName = channelName + ':next'
  const nextChannelVariable = formatChannelVariable(nextChannelName)
  const traceMethod = operator === 'traceAsyncIterator' ? 'tracePromise' : 'traceSync'

  transforms.tracingChannelDeclaration({ ...state, channelName: nextChannelName }, program)
    ...
    function wrapper () {
      if (!${channelVariable}.start.hasSubscribers) return __apm$traced();
      {
        ...
        const wrap = iter => {
          ...
          iter.next = (...args) => ${nextChannelVariable}.${traceMethod}(iterNext, ctx, iter, ...args);
          iter.return = (...args) => ${nextChannelVariable}.${traceMethod}(iterReturn, ctx, iter, ...args);
          iter.throw = (...args) => ${nextChannelVariable}.${traceMethod}(iterThrow, ctx, iter, ...args);
          return iter;
        };
