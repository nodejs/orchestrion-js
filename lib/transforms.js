...
/**
 * Returns `true` if `ctor` already contains the constructor-patch scaffolding for
 * `methodName`, i.e. a `const __apm$<methodName> = this["<methodName>"]`
 * declaration injected by {@link traceInstanceMethod}. Used to keep
 * re-transforming already instrumented output idempotent for instance methods
 * that are wrapped at runtime inside the constructor.
 *
 * @param {import('estree').MethodDefinition} ctor - The class constructor node.
 * @param {string} methodName
 * @returns {boolean}
 */
function constructorPatchesMethod (ctor, methodName) {
  const marker = `__apm$${methodName}`
  const body = ctor?.value?.body?.body
  if (!Array.isArray(body)) return false
  return body.some(stmt =>
    stmt.type === 'VariableDeclaration' &&
    stmt.declarations.some(decl => decl.id?.name === marker)
  )
}
...
function traceInstanceMethod (state, node, program) {
  ...
  // Idempotency guard: a prior pass injects the runtime patch into the
  // constructor, so skip if this method has already been patched there. This is
  // per-method (not per-channel): the patch reuses a `const __apm$<method>`
  // binding in the constructor, so a second wrapper for a different channel
  // cannot be nested here without a redeclaration. Runtime-patched instance
  // methods therefore support a single instrumentation.
  if (ctor && constructorPatchesMethod(ctor, methodName)) return
