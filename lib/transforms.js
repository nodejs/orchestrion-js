...
const transforms = module.exports = {
  ...
  tracingChannelImport ({ dcModule, moduleType }, node) {
    ...
    const dcPolyfill = moduleType === 'esm'
      ? `import tr_ch_apm_dcPolyfill from "${dcModule}"`
      : `const tr_ch_apm_dcPolyfill = require("${dcModule}")`
    const tracingChannel = 'const { tracingChannel: tr_ch_apm_tracingChannel } = tr_ch_apm_dcPolyfill'
    ...
    node.body.splice(
      index + 1,
      0,
      parse(dcPolyfill, options).body[0],
