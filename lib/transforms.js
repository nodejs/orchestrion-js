...
const transforms = module.exports = {
  ...
  tracingChannelImport ({ dcModule, sourceType }, node) {
    ...
    const tracingChannel = sourceType === 'module'
    ...
    node.body.splice(
      index + 1,
      0,
      parse(tracingChannel, { sourceType }).body[0],
      parse(hasSubscribers, { sourceType }).body[0]
    )
  },
...
/**
 * Builds the wrapper AST for a sync or async iterator-returning function.
 *
 * Injects an additional tracing channel for the iterator's `next`/`return`/`throw`
 * methods and wraps each with the appropriate tracing call.
 *
